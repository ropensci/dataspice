...
test_that("a basic crosswalk succeeds", {
  spice <- system.file(file.path("examples", "annual-escapement.json"), package = "dataspice")
...
test_that("crosswalk variables works", {
  ...
  # From memory
  spice <- jsonlite::read_json(system.file(file.path("examples", "annual-escapement.json"), package = "dataspice"))
  expect_is(crosswalk_variables(spice), "data.frame")
})
...
test_that("crosswalking creators works", {
  ...
  expect_is(crosswalk_creator(list("name" = "foo", "type" = "Organization")), "list")
})
